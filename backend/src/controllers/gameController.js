...
const updateScore = async (req, res) => {
  try {
    ...
    const holeIndex = game.scores.findIndex(
      (scoreObj) => scoreObj.holeNumber === hole
    );
    if (holeIndex === -1) {
      return res.status(400).json({ error: "Hole not found in the game." });
    }

    // Update the user's score for this hole
    game.scores[holeIndex].scores[userIndex] = score;
